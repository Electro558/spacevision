        ...
        try {
          const capture = await prisma.generationCapture.create({
            data: {
              ...
              prompt,
              conversationHistory: Array.isArray(conversationHistory) ? JSON.parse(JSON.stringify(conversationHistory)) : undefined,
              toolCalls: allToolCalls.length > 0 ? JSON.parse(JSON.stringify(allToolCalls)) : undefined,
              finalSceneState: sceneState ? JSON.parse(JSON.stringify(typeof sceneState === 'string' ? JSON.parse(sceneState) : sceneState)) : undefined,
              sceneData: JSON.parse(JSON.stringify(allToolCalls.filter(tc => tc.name === 'add_object').map(tc => tc.input))),
